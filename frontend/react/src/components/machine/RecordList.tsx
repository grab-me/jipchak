...
const RecordList = () => {
  ...
  return (
    <div className="w-full h-full flex flex-col p-[6%] bg-transparent relative overflow-hidden font-crayon">
      {/* 타이틀: 크레파스 외곽선 효과 적용 */}
      <h2 className="text-[clamp(24px,3.2vw,36px)] font-bold text-center text-yellow-400 mb-[8%] select-none [text-shadow:0.2vw_0.2vw_0_#1f2937,_-0.15vw_-0.15vw_0_#1f2937,0.15vw_-0.15vw_0_#1f2937,_-0.15vw_0.15vw_0_#1f2937]">
        나의 기록
      </h2>
          ...
          <>
            {[...records].reverse().map((record, index) => {
              ...
              return (
                ...
                >
                  {/* 크레파스 느낌의 카드 외곽선 레이어들 */}
                  <div className="absolute inset-0 border-[0.5vw] border-crayon-line opacity-40 pointer-events-none rounded-[255px_15px_225px_15px/15px_225px_15px_255px]" />
                  <div className="absolute inset-0 border-[0.5vw] border-crayon-line opacity-60 pointer-events-none rounded-[15px_225px_15px_255px/255px_15px_225px_15px] rotate-1" />

                  {/* 순번 배지 영역 */}
                  <div className="absolute -top-[12%] -left-[6%] z-10 flex flex-col items-center">
                    {success ? (
                      <>
                        <div className="relative w-[clamp(36px,5vw,50px)] aspect-square bg-gradient-to-br from-yellow-200 via-yellow-400 to-yellow-600 rounded-full flex items-center justify-center border-[0.3vw] border-yellow-600 shadow-[0_0.5vw_1.2vw_rgba(234,179,8,0.4),inset_0_0.2vw_0.5vw_rgba(255,255,255,0.7)] z-20">
                          <div className="absolute inset-[0.2vw] rounded-full border-[0.1vw] border-yellow-200/60 pointer-events-none" />
                        ...
                        <div className="flex -mt-[18%] gap-[0.2vw] z-10">
                          <div className="w-[1.5vw] h-[3.2vw] bg-gradient-to-b from-red-500 to-red-800 origin-top rotate-[25deg] rounded-b-[0.2vw] border-x-[0.1vw] border-b-[0.1vw] border-red-900 shadow-md" />
                          <div className="w-[1.5vw] h-[3.2vw] bg-gradient-to-b from-red-500 to-red-800 origin-top -rotate-[25deg] rounded-b-[0.2vw] border-x-[0.1vw] border-b-[0.1vw] border-red-900 shadow-md" />
                        </div>
                      </>
                    ) : (
                      <div className="relative w-[clamp(32px,4.5vw,44px)] aspect-square bg-white rounded-[255px_15px_225px_15px/15px_225px_15px_255px] border-[0.3vw] border-crayon-line shadow-md z-20 mt-[10%] flex items-center justify-center">
                  ...
                  {/* 파일명: 크레파스 느낌 살리기 */}
                  <span className="text-[clamp(14px,1.6vw,20px)] font-medium text-gray-800 break-words leading-tight select-none text-center [text-shadow:0.1vw_0.1vw_0_#ccc]">
            ...
            {records.length >= 3 && (
              <div className="w-full flex justify-center pt-[4%] pb-[12%] shrink-0">
                <button 
                  onClick={scrollToTop}
                  className="px-[clamp(24px,3vw,48px)] py-[clamp(10px,1.2vw,20px)] bg-gray-800 text-white rounded-[255px_15px_225px_15px/15px_225px_15px_255px] font-bold shadow-xl border-[0.2vw] border-white flex items-center gap-[1vw] active:scale-95 transition-transform"
                >
